...
<h3 class='example-title'>{$editor.title}{$word.marks}<if value="$data['n'] eq 'about'"><a href="{$url.site_admin}#/column" target="_blank" class="text-help ml-2">{$word.admin_colunmmanage_v6}</a></if></h3>
</if>
<dl>
	<if value="$editor['dt']">
	<dt>
		<label class='form-control-label'>{$editor.dt}{$word.marks}</label>
	</dt>
	</if>
	<dd class='clearfix'>
